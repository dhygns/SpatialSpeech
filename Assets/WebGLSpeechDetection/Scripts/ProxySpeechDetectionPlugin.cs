...
namespace UnityWebGLSpeechDetection
{
    public class ProxySpeechDetectionPlugin : BaseSpeechDetectionPlugin, ISpeechDetectionPlugin
    {
        ...
        public void ManagementLaunchProxy()
        {
			...
			try
			{
				string launch = Path.Combine(GetAppDataFolder(), "launch.sh");
				using (FileStream fs = File.Open(launch, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
				{
					using (StreamWriter sw = new StreamWriter(fs))
					{
						sw.WriteLine("bash -c \"osascript -e \\\"do shell script \\\\\\\"/Library/Frameworks/Mono.framework/Versions/5.0.1/bin/mono32 --debug {0}/{1} {2}\\\\\\\" with administrator privileges\\\"\"",
							speechProxyConfig.installDirectory,
							speechProxyConfig.appName,
							_mPort);
						sw.Flush();
				...
				Process process = new Process();
				ProcessStartInfo psi = new ProcessStartInfo(APP_BASH_MAC, launch);
